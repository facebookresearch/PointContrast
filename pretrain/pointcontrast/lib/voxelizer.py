...
class Voxelizer:
  ...
  def __init__(self,
    ...
    print("scale_augmentation_bound: ", self.scale_augmentation_bound)
    self.rotation_augmentation_bound = rotation_augmentation_bound
    print("rotation_augmentation_bound: ", self.rotation_augmentation_bound)
    self.translation_augmentation_ratio_bound = translation_augmentation_ratio_bound
    print("translation_augmentation_bound: ", self.translation_augmentation_ratio_bound)

  def sample_random_rot(self, rotation_range=360):
    R = M(np.random.rand(3) - 0.5, rotation_range * np.pi / 180.0 * (np.random.rand(1) - 0.5))
    return R

  def get_transformation_matrix(self, free_rot):
    ...
    rot_mat = np.eye(3)
    if free_rot:
        rot_mat = self.sample_random_rot()
    else:
  ...
  def voxelize(self, coords, feats, labels, center=None, free_rot=True):
    ...
    M_v, M_r = self.get_transformation_matrix(free_rot)
